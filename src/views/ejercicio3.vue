    ...
    <dai-tarjeta-contenedor-mapa :permitir-colapso="false">
      <template #header>
        <draggable v-model="capas" class="lista-capas">
          <div v-for="capa in capas" :key="capa" class="item-capa">{{capa}}</div>
        </draggable>
      </template>
      <dai-mapa :extension="[-117.6, 13.6, -86.5, 33.5]">
        <dai-capa-xyz-osm />
        <dai-capa-wms 
          v-for="capa in capas" :key="capa"
...
export default {
  ...
  data() {
    return {
      capas: ["aeropuertos_210429","cabeceras","corredores","estados_210521","hospitales_priv_210318"],
    };
  },
  computed: {
    zindexAsDict() {
